...
//' @export
// [[Rcpp::export]]
IntegerMatrix boot_xbal1_pips_rcpp_brewer(const NumericVector & x_sample,
                                          double x_pop_total,
                                          int n_boots) {
  int n = x_sample.size();
  ...
  for(int i = 0; i < n; i++) {
    ...
    r[i] = inv_prs[i] - ci[i];       // calculate r
  ...
  std::sort(idx.begin(), idx.end(), [&](int i, int j){return r[i] > r[j];});
  ...
  for(int row = 0; row < n_boots; row++) {
    ...
    IntegerVector sample = UPbrewer_rcpp(pr_b);
